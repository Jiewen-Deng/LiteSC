# -*- coding: utf-8 -*-
"""
Transformer-based Structure of quantization-based digital semantic communication system.
@Author: JW Deng
@Time: 2026/7/6 12:00
"""
"""
...
from N2UQ.utils_quantization import AveragedRangeTracker, AsymmetricQuantizer_constellation, SymmetricQuantizer_constellation
from N2UQ.utils_N2UQ import N2UQ_Symmetric_constellation, N2UQ_Asymmetric_constellation
...
class Transformer(nn.Module):
    def __init__(self, num_layers, src_vocab_size, trg_vocab_size, src_max_len, 
                 trg_max_len, d_model, num_heads, dff, dropout = 0.1,  quant = 'quant', quant_bits = 4):
        ...
        if quant == 'quant':
            self.quant_constellation = AsymmetricQuantizer_constellation(bits=quant_bits, range_tracker=AveragedRangeTracker(
                q_level='L'))
        elif quant == 'N2UQ_Symmetric':
            self.quant_constellation = N2UQ_Symmetric_constellation(bits=quant_bits, range_tracker=AveragedRangeTracker(q_level='L'))
        elif quant == 'N2UQ_Asymmetric':
            self.quant_constellation = N2UQ_Asymmetric_constellation(bits=quant_bits, range_tracker=AveragedRangeTracker(q_level='L'))
        else:
    ...
    def forward(self, src, n_var, pad, criterion, channel='AWGN', device = 'cpu'):
        ...
        channel_enc_output_quant = self.quant_constellation(channel_enc_output)
