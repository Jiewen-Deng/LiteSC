"""
This is the utils for uniform quantization.
@Author: JW Deng
@Time: 2026/7/6 16:44
"""
import torch
...
class Quantizer(nn.Module):
    def __init__(self, bits, range_tracker):
        ...
        self.register_buffer('scale', torch.tensor(0.))      # Quantization scale factor
        self.register_buffer('zero_point', torch.tensor(0.)) # Quantization zero point
...
class AsymmetricQuantizer(UnsignedQuantizer):
    ...
    def update_params(self):
        ...
        scale_value = quantized_range /  (float_range + 1e-8)
        self.scale.copy_(scale_value.squeeze())
        self.zero_point.copy_((torch.round(self.range_tracker.min_val * self.scale)).squeeze())
        # self.scale = quantized_range / (float_range + 1e-8)
        # self.zero_point = torch.round(self.range_tracker.min_val * self.scale)

"""
Quantizer for constellation diagrams, enabling fixed constellation diagram output
"""

class SymmetricQuantizer_constellation(SignedQuantizer):
    ...
    def update_params(self, input):
        ...
        self.scale.copy_(scale_value.squeeze())
        self.zero_point.copy_(torch.zeros_like(self.scale))
...
class AsymmetricQuantizer_constellation(UnsignedQuantizer):
